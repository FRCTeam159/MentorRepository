...
static const int  MAXPIDCHNLS = 2;

public class CANTalon implements SpeedController,ITableListener,LiveWindowSendable,PIDSource {
	...
	*/
	...
	void SetRaw(unsigned short value) {
		//wpi_assert(value == 0);
		impl->Set(value);
	}
	...
	bool IsModePID(ControlMode mode) {
	...
	void SetVelocity(double value) {
		pid_data[pid_channel].SetSetpoint(value);
		if (debug)
			std::cout << id << " SetVelocity:" << value << std::endl;
	}
	void SetPosition(double value) {
		if (debug)
			std::cout << id << " SetPosition:" << value << std::endl;
		pid_data[pid_channel].SetSetpoint(value);
	}
	...
	double GetPosition() const {
		if (encoder)
			...
			std::cout << "ERROR GetPosition() encoder=NULL" << std::endl;
			return 0;
		}
	}
	...
	void ConfigSoftPositionLimits(double forward, double reverse) {
	...
	void SetEncPosition(int int1) {
		// TODO: set raw encoder ticks ?
	}
	
	int GetClosedLoopError() const {
	...
	class PIDData {
		...
		void SetPID(double P, double I, double D, PIDSource s,
				PIDOutput d) {
			SetP(P);
			SetI(I);
			SetD(D);
